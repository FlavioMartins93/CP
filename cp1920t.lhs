...
\\
a57754 & Luís Gonçalo Ferreira Mendes
\\
e3010 & Pedro Santos Arezes Costa
\end{tabular}
\end{center}
...
discollect x = concat [ [(k,a) | a<-d] | (k,d) <-x]
...
\xymatrix@@C=5cm{
    |(S >< S) >< Dict|
           \ar[d]_-{||(g)||}
           \ar[r]_-{|g = (p2 + f) . distr . ((id >< id) >< outExp)|}
&
    | S + (S >< ((S >< S) >< Dict)*)|
           \ar[d]^{|F (anaNat (g)) |}
\\
...
&
     | S + (S >< Dict*) |
           \ar[l]^-{|f = inExp . (id + (id >< (map inExp . set . map outExp)))|}
}
\end{eqnarray*}
...
&
     |S + (S >< S**)|
           \ar[l]^-{|f = [singl , concat . p2]|}
}
\end{eqnarray*}

\begin{code}

dic_rd p d = Cp.cond ((==0) . length) (const Nothing) (Just . set) $ hyloExp cg ag (d,p) where
  ag = (either v (i2 . k))  . distl . (outExp >< id)  
  k ((s1,d),s2) 
    | s1 == "" = (s1, map (\x -> (x,s2)) d)
    | length s2 > 0 && head s1 == head s2 = (s1, map (\x -> (x,tail s2)) d)
    | otherwise = (s1,[])
  v (s2,"") = i1 s2
  v (s2,_) = i2 ("",[])
  cg = either singl ( concat . p2)
...
\xymatrix@@C=5cm{
    |BTree A|
           \ar[d]_-{|maisDir = (cataNat g)|}
           \ar[r]_-{|outBTree|}
&
    |1 + (A >< ((BTree A) >< (BTree A)))|
           \ar[d]^{|F (cataNat g) = id + ( id >< ((cataNat g) >< (cataNat g)))|}
\\
    |1 + A|
&
    |1 + (A >< ((1+A) >< (1+A)))|
...
maisDir = cataBTree g where
  g = either (const Nothing) gdir 
  gdir (a,(_,Nothing)) = Just a
  gdir (a,(_,d)) = d
...
maisEsq = cataBTree g where
  g = either (const Nothing) gesq 
  gesq (a,(Nothing,_)) = Just a
  gesq (a,(e,_)) = e
...
\xymatrix@@C=3.8cm{
    |BTree A|
          \ar[d]_-{|cataNat (split h k)|}
          \ar[r]_-{|outBTree|}
          \ar@@/^-7pc/[dd]^{|p1 . cataNat (split h k)|}
&
    |1 + (A >< (BTree A >< BTree A))|
          \ar[d]^{|F (cataNat (split k h)|}
\\
    |BTree A >< BTree A|
          \ar[d]^{|p1|}
&
...
insOrd a = p1 . (cataBTree (split (either (const (Node (a,(Empty,Empty)))) (chooseSide)) (either (const Empty) (Node . (id >< (p2 >< p2)))))) where
  chooseSide (v,((ee,ed),(de,dd)))
    | a > v = (Node(v,(ed,de)))
    | otherwise = (Node(v,(ee,dd)))
...
\xymatrix@@C=4cm{
    |BTree A|
          \ar[d]_-{|cataNat (split h k)|}
          \ar[r]_-{|outBTree|}
          \ar@@/^-6pc/[dd]^{|p1 . cataNat (split h k)|}
&
    |1 + (A >< (BTree A >< BTree A))|
          \ar[d]^{|F (cataNat (split k h)|}
\\
...
isOrd = p1 . (cataBTree (split (either true noOrd) (either (const Empty) reconstructBT)))

reconstructBT :: (a,((Bool,BTree a), (Bool,BTree a))) -> BTree a
reconstructBT (a,((bd,d),(br,e))) = Node(a,(d,e))

-- Nodo ordenado
noOrd :: (Ord a) => (a,((Bool,BTree a), (Bool,BTree a))) -> Bool
noOrd (a,((bd,e),(br,d))) 
    | bd == False || br == False = False
    | maisDir d == Nothing && maisEsq e == Nothing = True
    | maisDir d == Nothing = Just a >= maisDir e
    | maisEsq e == Nothing = Just a <= maisEsq d
    | otherwise = Just a >= maisDir e && Just a <= maisEsq d
...
\\
    |(expn((BTree A)) (expn(2) (*)))|
&
    |1 + (A >< ((expn((BTree A)) (expn(2) (*)))><(expn((BTree A)) (expn(2) (*)))))|
          \ar[l]^-{|g|}
}
\end{eqnarray*}
...
splay = flip $ cataBTree $ either (const . const Empty) apRot where
  apRot (a,(e,d)) = Cp.cond ((==0) . length) (Node . split (const a) (split e d)) (k (a,(e,d)))
  k (a,(e,d)) = Cp.cond head (rrot . Node . split (const a) (split (e . tail) (d . nil))) (lrot . Node . split (const a) (split (e . nil) (d . tail)))
...
outBdt (Dec a) = Left(a) 
outBdt (Query(a,(b1,b2))) = Right(a,(b1,b2))
...
\xymatrix@@C=5cm{
    |LTree A|
          \ar[d]_-{|navLTree = (cataNat g)|}
          \ar[r]_-{|outLTree|}
&
    |A + (LTree A >< LTree A)|
          \ar[d]^{|F (cataNat g) = id + ((cataNat g) >< (cataNat g))|}
\\
    |expn (LTree A) (expn (2) (*))|
&
    |A + ((expn (LTree A) (expn (2) (*))) >< (expn (LTree A) (expn (2) (*))) )|
          \ar[l]^-{|g|}
}
\end{eqnarray*}
...
navLTree = cataLTree $ either (const . Leaf) nav where
  nav (e,d) = Cp.cond ((==0) . length) (Fork . (split e d)) (Cp.cond head (e . tail) (d . tail))
...
&
    |A + (LTree A >< LTree A)|
          \ar[d]^{|F (cataNat g) = id + ((cataNat g) >< (cataNat g))|}
\\
    |expn (LTree A) (expn (BTree 2) (*))|
&
    |A + ((expn (LTree A) (expn (BTree 2) (*))) >< (expn (LTree A) (expn (BTree 2) (*))) )|
          \ar[l]^-{|g|}
}
\end{eqnarray*}
...
bnavLTree = cataLTree $ either (const . Leaf) nav where
  nav (e,d) = (either (Fork . split e d . const Empty) (Cp.cond p1 (e . p1 . p2) (d . p2 . p2))) . outBTree
...
&
    |A + (LTree A >< LTree A)|
          \ar[d]^{|F (cataNat g) = id + ((cataNat g) >< (cataNat g))|}
\\
    |expn (Dist (LTree A)) (BTree (Dist 2))|
&
    |A + ((expn (Dist (LTree A)) (BTree (Dist 2))) >< (expn (Dist (LTree A)) (BTree (Dist 2))) )|
          \ar[l]^-{|g|}
}
\end{eqnarray*}
...
lt = Fork(Fork(Leaf "Precisa",Fork(Leaf "Precisa",Leaf "N precisa")),Leaf "N precisa")

bt :: BTree (Dist Bool)
bt = Node(prob1,(esq,Empty))
esq = Node(prob2,(Empty,Node(prob3,(Empty,Empty))))

prob1 = D [(True,0.15),(False,0.85)]
prob2 = D [(True,0.8),(False,0.2)]
prob3 = D [(True,0.6),(False,0.4)]

out =  D [
        --((Fork(Leaf "Precisa",Fork(Leaf "Precisa", Leaf "Nao precisa"))) , 0.15),
        (Leaf "Precisa", 0.80*0.15),
        --(Fork(Leaf "Precisa", Leaf "Nao precisa"), 0.2*0.15),
        (Leaf "Precisa", 0.6*0.2*0.15),
        (Leaf "N Precisa", 0.4*0.2*0.15),
        (Leaf "N Precisa",0.85)
        ]

apPr :: (Dist a,Probability) -> Dist a
apPr (d,(P p)) = D (map (\(a,b) -> (a,(b*p))) (unD d))

joinDists :: Dist a -> Dist a -> Dist a
joinDists a b = D ((unD a) ++ (unD b))

headDist :: Dist a -> a
headDist = p1 . head . unD

pbnavLTree = cataLTree $ either (const . return . Leaf) g where 
  g (e,d) = ( either (const $ return (Fork ((headDist $ e Empty),(headDist $ d Empty)))) (h (e,d))) . outBTree
  h (e,d) = uncurry joinDists . split (apPr . split (p1 . p2) (p1 . p1)) (apPr . split (p2 . p2) (p2 . p1)) . ( (split ((??) id) ((??) not) . norm ) >< ( e >< d ) )
...
-------------------------------------------------
chooseTruchet = fmap (Cp.cond (==1) (const truchet1) (const truchet2)) $ randomRIO(0::Integer,1::Integer)
generator (a,b) = do 
  { x <- [-a/2,(-a/2+80)..a/2-80]; y <- [-b/2,(-a/2+80)..b/2-80]; return (fmap (put (x,y)) chooseTruchet) }

main = fmap Pictures (sequence (generator (800,800))) >>= display janela white 

\end{code}

\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.75]{images/truchet_gerado.png}
    \caption{Mosaico de Truchet-Smith gerado.}
    \label{fig:truchet}
    \end{figure}
